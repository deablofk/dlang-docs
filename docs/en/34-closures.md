...
// its captured environment is heap-allocated from the current allocator.
fazContador :: () -> Ptr((() -> int)) {
  var n = 0
...
The heart of the design is the escape distinction. A non-escaping closure is free (stack); an escaping one heap-allocates its captured environment, drawing that memory from the current allocator (the same ambient context every other allocation uses — see [Dynamic Allocation](18-dynamic-allocation.md)). The cost is real but predictable, and you can redirect it by installing a different allocator.
